...
def merge_config(defaults, plugin_params1, plugin_params2, file_config, cli_args, unknown_args):
    ...
    if not _QUIET: print(f"Actual Step 1 Output: {merged_config}")

    # Step 2: Merge with plugin_params1
    for k, v in plugin_params1.items():
        if not _QUIET: print(f"Step 2 merging: plugin_param1 {k} = {v}")
        merged_config[k] = v

    # Step 2.5: Merge with plugin_params2
    for k, v in plugin_params2.items():
        if not _QUIET: print(f"Step 2.5 merging: plugin_param2 {k} = {v}")
        merged_config[k] = v

    # Step 3: Merge with file configuration
    for k, v in file_config.items():
        if not _QUIET: print(f"Step 3 merging from file config: {k} = {v}")
        merged_config[k] = v

    if not _QUIET: print(f"Actual Step 3 Output: {merged_config}")

    # Step 4: Merge with CLI arguments (ensure CLI args always override)
    cli_keys = [arg.lstrip('--') for arg in sys.argv if arg.startswith('--')]
    for key in cli_keys:
        if key in cli_args:
            if not _QUIET: print(f"Step 4 merging from CLI args: {key} = {cli_args[key]}")
            merged_config[key] = cli_args[key]
        elif key in unknown_args:
            value = convert_type(unknown_args[key])
            if not _QUIET: print(f"Step 4 merging from unknown args: {key} = {value}")
    ...
    if not _QUIET: print(f"Actual Step 4 Output: {merged_config}")
