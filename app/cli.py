...
def parse_args():
    ...
    # use_hourly
    parser.add_argument('--use_hourly', action='store_true', help='Use hourly predictions directory to save the results instead of the daily if predictor config files are provided only.')
    ...
    parser.add_argument('--prediction_interval_minutes', type=int, help='How often predictions refresh in minutes (0=every bar, 240=every 4h on 1h bars).')
    ...
    parser.add_argument('--use_first_match', action='store_true', help='Use first match  or use best match for opening.')
